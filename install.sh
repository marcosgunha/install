...
# Description: Install and manage a ExiZap installation.
# OS: Ubuntu 20.04 LTS
...
function get_domain_info() {
  read -rp 'Enter the domain/subdomain for ExiZap (e.g., ExiZap.domain.com): ' domain_name
...
serve ExiZap at https://$domain_name.
...
##############################################################################
# Configure postgres to create ExiZap db user.
...
function setup_chatwoot() {
  ...
  git clone https://github.com/marcosgunha/chatwoot.git
  cd chatwoot
  git checkout "$BRANCH"
  bundle
  yarn
...
##############################################################################
# Setup ExiZap systemd services and cwctl CLI
# Globals:
#   None
# Arguments:
#   None
...
***************************************************************************
*****************ExiZap server installation is complete.*******************
...
function get_cw_version() {
  CW_VERSION=$(curl -s https://app.exizap.com.br/api | python3 -c 'import sys,json;data=json.loads(sys.stdin.read()); print(data["version"])')
}
...
EOF
  ...
  sleep 3
  read -rp 'Would you like to configure a domain and SSL for ExiZap?(yes or no): ' configure_webserver
  ...
  echo "➥ 6/9 Installing ExiZap. This takes a long while."
...
##############################################################################
# Get ExiZap web/worker logs (-l/--logs)
# Globals:
#   None
# Arguments:
#   None
...
function upgrade() {
  get_cw_version
  echo "Upgrading ExiZap to v$CW_VERSION"
  sleep 3
  upgrade_prereq
  sudo -i -u chatwoot << "EOF"
...
EOF
  ...
  # Restart the ExiZap server
  systemctl restart chatwoot.target

}

##############################################################################
# Restart ExiZap server (-r/--restart)
# Globals:
#   None
# Arguments:
#   None
